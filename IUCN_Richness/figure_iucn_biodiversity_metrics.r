...
for (grid_index in c(1:10)){
  ...
  for (seed_i in 1:nrow(selected_seeds)){
    print(paste(res, var, grid_index, seed_i))
...
#for raw richness
all_points_sub<-all_points_full[type=="RAW"]
source("colors.r")
...
#for full richness
all_points_sub<-all_points_full[type=="FULL"]
p<-ggplot(all_points_sub)+geom_tile(aes(x=x, y=y, fill=v))+
...
p<-ggplot(all_points_sub)+geom_tile(aes(x=x, y=y, fill=v))+
  ...
  facet_wrap(~grid_label, nrow=2, ncol=5)
p
ggsave(p, filename="../Figures/IUCN_Based_VW/random_sampling_richness.png", width=10, height=5)
...
p<-ggplot(all_points_sub[grid_label==g_label])+
  ...
  scale_fill_gradient(low=colors_blue[6], high=colors_red[6])+
  map_theme+
  facet_wrap(~seed_label, nrow=2, ncol=5)
p
ggsave(p, filename=sprintf("../Figures/IUCN_Based_VW/road_based_grid_%s_richness.png", g_label), width=10, height=5)
